		...
		<div class="grid grid-cols-2 gap-4 md:grid-cols-4 2xl:gap-6">
			...
			<div class="wow flipInY col-span-1" data-wow-delay="{{ ($key + 1) * 0.1 }}s">
				<div class="item-reason rounded text-center">
					<span class="img img__ c-img lg::mb-4 mb-2 block overflow-hidden rounded-2xl pt-[90%] lg:pt-[108%]">
						@if(!Support::isMobile())
							@include('image_loader.tiny', ['itemImage' => $itemReasonChoose, 'key' => 'img', 'noLazyLoad' => 1])
						@else
							@include('image_loader.tiny', ['itemImage' => $itemReasonChoose, 'key' => 'img'])
						@endif
					</span>
					<h3 class="title mb-2 text-center text-[0.875rem] sm:text-[1rem] font-bold uppercase text-[#000] lg:mb-4 lg:text-[1.125rem]">{{ Support::show($itemReasonChoose, 'name') }}</h3>
					<div class="short_content text-justify leading-[1.5] lg:leading-[1.5]">{!! Support::show($itemReasonChoose, 'content') !!}</div>
				</div>
			</div>
			@endforeach
		</div>
...
<section class="section-service__index relative bg-cover bg-no-repeat py-6 after:absolute after:top-0 after:left-0 after:z-[1] after:h-full after:w-full after:bg-[rgba(255,255,255,.5)] 2xl:py-10" style="background-image: url({Ibg_service_home.imgI});">
	...
	{{--
		...
		--}}
		<div class="grid gap-4 grid-cols-2 gap-4 md:grid-cols-4 2xl:gap-6">
			@foreach ($listHotService as $key => $item)
			<div class="wow fadeInUp col-span-1" data-wow-delay="{{ ($key + 1) * 0.1 }}s">
				@include('services.item',['page'=>'home'])
			</div>
			@endforeach
		</div>
	</div>
...
<section class="section-faq__index bg-cover bg-no-repeat py-6 lg:py-14 2xl:py-20" style="background-image: url({Ibg_frequent_question_home.imgI});">
	<div class="container">
		...
		@endphp
		<div class="grid grid-cols-1 sm:gap-4 gap-2 lg:grid-cols-5 lg:gap-8 2xl:gap-20">
			<div class="wow fadeInLeft col-span-1 lg:col-span-3">
				@if (isset($bigItemQuestion))
				<div class="item-new__main">
					<a class="img img__ c-img mb-2 sm:mb-4 block overflow-hidden rounded-2xl pt-[55%]" href="{{ Support::show($bigItemQuestion, 'slug') }}" title="{{ Support::show($bigItemQuestion, 'name') }}">
						@include('image_loader.all', ['itemImage' => $bigItemQuestion, 'key' => 'img'])
					</a>
					<p>
						<a class="title line-clamp-2 mb-2 text-[1rem] font-bold uppercase text-white lg:text-[1.125rem] 2xl:mb-4" href="{{ Support::show($bigItemQuestion, 'slug') }}" title="{{ Support::show($bigItemQuestion, 'name') }}">{{ Support::show($bigItemQuestion, 'name') }}</a>
					</p>
					<div class="short_content line-clamp-2 mb-2 flex-1 text-white 2xl:mb-4">{{ Support::show($bigItemQuestion, 'question') }}</div>
					<a class="btn-white inline-flex items-center justify-center rounded-3xl bg-white py-2 px-4 text-[#262626] shadow-[0_6px_8px_rgba(0,0,0,.17)] transition-all duration-300 lg:text-[0.875rem]" href="{{ Support::show($bigItemQuestion, 'slug') }}" title="Xem thêm">
						Xem thêm
			...
			<div class="wow fadeInRight col-span-1 lg:col-span-2">
				<h2 class="title-all relative !mb-2 sm:mb-4 text-center text-[1.25rem] font-semibold uppercase text-white lg:pl-3 lg:text-left lg:text-[2rem] lg:before:absolute lg:before:left-0 lg:before:top-1/2 lg:before:h-[80%] lg:before:w-1 lg:before:-translate-y-1/2 lg:before:bg-white 2xl:mb-8 2xl:text-[2rem]">
				...
				<div class="module-faq lg:max-h-[415px] lg:overflow-y-auto 2xl:max-h-[530px]">
					...
					@if ($key > 0)
					<div class="item-faq mb-2 sm:mb-4 last:mb-0 2xl:mb-5">
						<p class="question mb-2 sm:mb-3 text-[0.875rem] text-white 2xl:text-[1.125rem]">
						...
						<a class="btn-white inline-flex items-center justify-center rounded-3xl bg-white py-2 px-6 text-[#262626] shadow-[0_6px_8px_rgba(0,0,0,.17)] transition-all duration-300 lg:text-[0.875rem]" href="{{ Support::show($itemSmallQuestion, 'slug') }}" title="Xem thêm">
							Trả lời
							<i aria-hidden="true" class="fa fa-angle-double-right ml-2"></i>
						</a>
					</div>
					...
					<div class="swiper-slide">
						<div class="item-system">
							...
							<div class="system-info p-4">
								<p class="item relative mb-2 sm:mb-3 pl-6 last:mb-0">
...
<section class="section-new__index py-6 !pt-0 2xl:py-10">
	<div class="container">
		...
		<div class="module-new__index grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:gap-6">
			...
			<div class="wow fadeInLeft col-span-1">
				<div class="item-new__main">
					<a class="btn-play-video img img__ c-img mb-2 block overflow-hidden rounded-lg pt-[54%] lg:rounded-2xl 2xl:mb-4" data-fslightbox="video" href="{{ Support::show($videoHome, 'link_video') }}" title="{{ Support::show($videoHome, 'name') }}">
						@include('image_loader.all', ['itemImage' => $videoHome, 'key' => 'img'])
					</a>
					<p>
						<a class="title line-clamp-2 mb-2 text-[0.875rem] sm:text-[1rem] font-bold uppercase text-[#262626] lg:text-[1.25rem] 2xl:mb-4" href="{{ Support::show($videoHome, 'link') }}" title="{{ Support::show($videoHome, 'name') }}">{{ Support::show($videoHome, 'name') }}</a>
					</h3>
					<div class="short_content line-clamp-2 mb-2">{{ Support::show($videoHome, 'content') }}</div>
					<a class="btn-white inline-flex items-center justify-center rounded-3xl border-[1px] border-solid border-[#028cde] bg-white py-1 px-3 text-[#262626] shadow-[0_6px_8px_rgba(0,0,0,.17)] transition-all duration-300 hover:bg-[#028cde] hover:text-white lg:py-2 lg:px-6 lg:text-[0.875rem]" href="{{ Support::show($videoHome, 'link_video') }}" title="Xem thêm">
						Xem thêm
						<i aria-hidden="true" class="fa fa-angle-double-right ml-2"></i>
					</a>
				</div>
			...
			<div class="wow fadeInRight col-span-1">
				...
				<div class="list-new__item mb-2 sm:mb-4 flex gap-3 last:mb-0 2xl:mb-6">
					<a class="img img__ block h-[70px] w-[125px] shrink-0 overflow-hidden rounded sm:h-[100px] sm:w-[180px] lg:rounded-lg" href="{{ Support::show($itemSmall, 'slug') }}" title="{{ Support::show($itemSmall, 'name') }}">
						@include('image_loader.tiny', ['itemImage' => $itemSmall, 'key' => 'img'])
					</a>
					<div class="new-content">
						<p>
							<a class="title line-clamp-2 font-bold uppercase 2xl:text-[1.125rem]" href="{{ Support::show($itemSmall, 'slug') }}" title="{{ Support::show($itemSmall, 'name') }}">{{ Support::show($itemSmall, 'name') }}</a>
						</p>
						<div class="short_content line-clamp-2 mb-2">{{ Support::show($itemSmall, 'short_content') }}</div>
						<a class="btn-white ml-auto mr-0 flex w-fit items-center justify-center rounded-3xl border-[1px] border-solid border-[#028cde] bg-white py-1 px-3 text-[#262626] shadow-[0_6px_8px_rgba(0,0,0,.17)] transition-all duration-300 hover:bg-[#028cde] hover:text-white lg:py-2 lg:px-6 lg:text-[0.875rem]" href="{{ Support::show($itemSmall, 'slug') }}" title="Xem thêm">
							Xem thêm
							<i aria-hidden="true" class="fa fa-angle-double-right ml-2"></i>
						</a>
					</div>
...
@endif
<a class="callback fixed top-1/2 right-[10px] z-10 hidden -translate-y-1/2 items-center lg:flex" data-modal="modalRegis" href="javascript:void(0)" onclick="MORE_FUNCTION.showModal(this);" title="Yêu cầu gọi lại">
	...
	<div class="relative mx-auto h-full w-full max-w-[370px] rounded-[1.25rem] border-[1px] border-solid border-[#5e8fea] bg-white p-2 md:h-auto">
		<form action="" class="overflow-hidden rounded-[1.25rem] bg-white shadow-[6px_8px_40px_rgba(0,0,0,.2)]" method="">
			...
			<div class="content py-3 px-4">
				...
				<button class="btn-red mb-3 inline-flex w-full items-center justify-center bg-[#fb2317] py-2 px-4 text-[14px] font-semibold uppercase text-white transition-all duration-300 md:text-[18px] lg:text-[22px]">Hoàn
					tất</button>
