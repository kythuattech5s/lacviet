    ...
    <div class="nav-category">
        ...
        <ul class="nav-service">
            @foreach ($listCate as $itemCate)
                <li>
                    <a href="{{ Support::show($itemCate, 'slug') }}" title="{{ Support::show($itemCate, 'name') }}" class="2xl:text-[1.4rem] text-[1rem] uppercase block py-2 font-bold text-[#028cde] lg:text-[1.25rem]">{{ Support::show($itemCate, 'name') }}</a>
