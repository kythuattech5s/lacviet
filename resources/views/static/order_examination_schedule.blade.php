            ...
            <form action="{{VRoute::get('bookApointment')}}" method="post" class="form-booking form-validate" absolute data-success="NOTIFICATION.toastrMessageRedirect" accept-charset="utf8">
                ...
                <span class="title whitespace-nowrap">Dịch vụ cần tư vấn: </span>
                    <select name="service" class="flex-1 text-center form-control bg-white lg:p-3 px-3 py-2 lg:rounded-xl rounded-lg w-full border-[1px] border-solid border-[#e0eaf0]">
                        ...
                        <option value="{{$item->id}}" data-name="{{$item->name}}" {{isset(request()->service) && request()->service == $item->id ? 'selected':''}}>{{$item->name}}</option>
                        @endforeach
                    </select>
                    <input type="hidden" value="" name="service_text">
                </div>
