...
var MAIN = (function () {
        ...
        function beforeUnload() {
            window.onbeforeunload = function (e) {
                var confirmationMessage =
                    "Không thể thay đổi ở trình duyệt mới";
    ...
    var getLink = function () {
        ...
        $("button.get-link").click(function () {
            copyLink($(this).data("slug"));
        });
        function copyLink(slug) {
            var textarea = document.createElement("textarea");
            textarea.textContent = window.location.origin + "/" + slug;
...
var USERONLINE = (function () {
    ...
    var pusher = new Pusher("c854f3b115ff80b9e6c9", {
        cluster: "ap1",
    });
    ...
    function loadUser(data) {
        ...
        if (main !== null && typeof main) {
            main.querySelector("ul.h-user__list").innerHTML = html
                .slice(0, 5)
                .join("");
            main.querySelector(".count").innerHTML =
    ...
    function buildContent() {
        ...
        if (pathname.includes("/esystem/edit/configs/0")) {
            var content = JSON.stringify(
                "đang sửa " + (doing ? doing.innerText : "không xác định")
            );
            ...
            var content = JSON.stringify(
                "đang sửa " + (inputName ? inputName.value : "không xác định")
            );
        } else if (pathname.includes("/esystem/insert")) {
            var content = JSON.stringify(
                "đang thêm mới " + (doing ? doing.innerText : "không xác định")
            );
        } else if (pathname.includes("/esystem/copy")) {
            const inputName = document.querySelector('input[name="name"]');
            var content = JSON.stringify(
                "đang copy " + (inputName ? inputName.value : "không xác định")
            );
        } else if (
            pathname.includes("/esystem/view") ||
            pathname.includes("/esystem/search")
        ) {
            var content = JSON.stringify(
                "đang ở trang " + (doing ? doing.innerText : "không xác định")
            );
        } else {
    ...
    function clickShow() {
        ...
        count.onclick = function () {
            const list = document.querySelector(
                ".h-user-online .h-user__list-all"
            );
