jQuery(function($) {
    $(document).on('click', '.mdi-img[rel="mdi"]', function(event) {
    	...
    	$(this).parents('.media-item').children('.mdi-check').find('input[type="checkbox"]').trigger('click');
    ...
    $("a.preview").fancybox({ speedIn       :   600, 
			speedOut      :   200, 
			overlayShow   :   false,
			autoScale: true});
...
$(document).bind("contextmenu", function (event) {
    ...
    try{
      ...
      if(extimgs.indexOf(extra.extension)!=-1){
        ...
        str+="<li onclick='MediaManager.getDetailFile("+objfile.id+");return false;' data-action='editimage'><i class='fa fa-edit'></i>Chỉnh sửa ảnh</li>";  
      }
...
var $grid=$('.media-content .row').isotope({
 ...
 // 	var qsRegex;
	// var $input = $('.media-bar-s > input').keyup( debounce( function() {
	//   	qsRegex = new RegExp( $input.val(), 'gi' );
	//   	$grid.isotope({
	//   		filter: function() {
	//     		return qsRegex ? $(this).find('.mdi-title').text().match( qsRegex ) : true;
	//   		}
	//   	});
	// }, 100));
    $(document).on('submit', '.media-bar-s', function(event) {
        event.preventDefault();
  ...
  // 		});
        ...
        $('.media-bar-s input[name=keyword]').val('');
        $('.media-bar-s input[name=date_search]').val('');
        $(".media-content>.row").html('');
        ENLESS_PAGE.finish = false;
        ENLESS_PAGE.loadpage(0);
	})
...
ENLESS_PAGE.loadpage= function(index){
    if(ENLESS_PAGE.loading == false && !ENLESS_PAGE.finish){
        ...
        var baseurl = window.location.origin + window.location.pathname+location.search;

        var sort = $('.media-sort a.active');
        var inputKey = $('.media-bar-s input[name=keyword]');
        var inputDate = $('.media-bar-s input[name=date_search]');
        if (sort.length > 0) {
        ...
        if (inputKey.length > 0) {
            if (baseurl.indexOf('?') > -1){
                baseurl = baseurl+'&keyword='+inputKey.val();
            }else {
                baseurl = baseurl+'?&keyword='+inputKey.val();
            }
        }
        if (inputDate.length > 0) {
            if (baseurl.indexOf('?') > -1){
                baseurl = baseurl+'&date_search='+inputDate.val();
            }else {
                baseurl = baseurl+'?&date_search='+inputDate.val();
            }
        }
        $.get( ENLESS_PAGE.updateQueryStringParameter(baseurl,'page',index), function(data){
            ENLESS_PAGE.loading = false; 
            ...
            $('.loading-info').hide(); //hide loading animation once data is received
           	$(".media-content>.row").append(data);
           	if($('#view').attr('data-view')=="box"){
				$('.media-item').parent().addClass('media-item-l');
				$(".media-content>.row").isotope();
			}
			else{
				$('.media-item').parent().removeClass('media-item-l');
				$(".media-content>.row").isotope();
