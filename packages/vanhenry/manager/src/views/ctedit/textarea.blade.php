...
<div class="form-group">
 	...
 	<textarea  {{FCHelper::ep($table,'require')==1?'required':''}}  name="{{$name}}" placeholder="{{FCHelper::ep($table,'note')}}" dt-type="{{FCHelper::ep($table,'type_show')}}" class="form-control" rows="3">{{$value}}</textarea>
</div>
