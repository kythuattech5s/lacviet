...
trait ViewTrait{
	...
	**/
	...
	public function view_normal($table,$tableData,$tableDetailData){
        //Thông tin bảng
        $data['tableData'] = collect($tableData);
		$data['user'] = \Auth::guard('h_users')->user()->group;
		
		$data['tableData'] = $this->checkPermisionUser($table,$data,$tableData);
	...
	public function checkPermisionUser($table,$data,$tableData){
		$h_group_module = DB::table('h_group_modules')->where('table_map',$table)->first();
		$id_main = $h_group_module->id;
		$user = \Auth::guard('h_users')->user();
		$h_roles = DB::table('h_roles')->where('group_module_id',$id_main)->where('group_user_id',$user->group)->first();
		
		$permis = decbin($h_roles->role);
		if(isset($permis[1]) && (int) $permis[1] === 1 && $data['tableData']['has_insert'] == 1){
			$data['tableData']['has_insert'] = 1;
		}else{
			$data['tableData']['has_insert'] = 0;
		}

		if(isset($permis[2]) && (int) $permis[2] === 1 && $data['tableData']['has_update'] == 1){
			$data['tableData']['has_update'] = 1;
		}else{
			$data['tableData']['has_update'] = 0;
		}	
		
		if(isset($permis[3]) && (int) $permis[3] === 1 && $data['tableData']['has_delete'] == 1){
			$data['tableData']['has_delete'] = 1;
		}else{
			$data['tableData']['has_delete'] = 0;
		}
		if(isset($permis[4]) && (int) $permis[4] === 1 && $data['tableData']['has_copy'] == 1){
			$data['tableData']['has_copy'] = 1;
		}else{
			$data['tableData']['has_copy'] = 0;
		}
