...
class CheckController extends BaseAdminController{
    ...
    public function checkEditing(Request $request, $id){
        ...
        if($news_editing !== null){
            $date = new \DateTime;
            if ($news_editing->updated_at > $date->modify('-10 seconds')) {
                if ($news_editing->session_id == session()->getId() && $news_editing->h_user_id == $auth->id && $news_editing->tab_time == $request->tab_time) {
    ...
    public function checkHasEdit(Request $request, $id){
        $date = new \DateTime;
        $news_editing = NewsEditOnline::where('news_id', $id)->first();
        if($news_editing == null || ($news_editing !== null && $news_editing->updated_at <= $date->modify('-10 seconds'))){
            if($news_editing !== null){
                $news_editing->delete();
            }
            return response()->json([
