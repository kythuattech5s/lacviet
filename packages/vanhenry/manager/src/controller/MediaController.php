...
{	
		...
		if(array_key_exists("folder", $prs)){
			...
			if(strlen($inputpath)>0 && count($folders)>0){
				...
				if($f->count()>0 && $dbpath==$inputpath){
					...
					$data["nums"] = $nums;
					$data["listItems"] = Media::where("parent",$f[0]->id)
												->when($keyword,function($q,$keyword){
										            $q->where('name','like','%'.$keyword.'%');
										        })
										        ->when($date_search,function($q,$date_search){
										        	$dateCreatete = \Carbon\Carbon::createFromFormat("Y-m-d", $date_search);
										        	$startDay = $dateCreatete->startOfDay();
													$endDay   = $startDay->copy()->endOfDay();
										            $q->where('created_at','>',$startDay);
										            $q->where('created_at','<',$endDay);
										        })
												->where("trash",0)->orderBy("is_file","asc")->orderBy($orderContent['key'],$orderContent['type'])->paginate(MEDIA_PER_PAGE);
				}
				else{
			...
			$data["listItems"] = Media::where("parent",0)
										->when($keyword,function($q,$keyword){
								            $q->where('name','like','%'.$keyword.'%');
								        })
								        ->when($date_search,function($q,$date_search){
	...
	private function getInfoFile($filename,$file_path){
		...
		$obj->size_ord= filesize($file_path);
	...
	private function insertImageMedia($path, $filename,$parent = 0){
		...
		$m->parent = $parent;
		$m->path = $path;
		$extraInfo = $this->getInfoFile($filename,$path.$filename);
		$m->size = isset($extraInfo->size_ord) ? (int)$extraInfo->size_ord:0;
		$m->extra = json_encode($extraInfo);
		$m->save();
		return $m->id;
	}
	...
	public function uploadFileWm(Request $request){
		...
		if ($validator->fails()) {
            ...
            $position = \vanhenry\helpers\helpers\SettingHelper::getSetting('WM_LOGO_POSITION');
