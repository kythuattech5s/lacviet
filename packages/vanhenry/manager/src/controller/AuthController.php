    ...
    */
    ...
    public function authenticate()
    {
        ...
        if (Auth::guard($this->guard)->attempt($credentials)) {
            $dataLogin['h_user_id'] = Auth::guard($this->guard)->user()->id;
            $dataLogin['info'] = json_encode($_SERVER);
            $dataLogin['user_ip'] = $_SERVER['REMOTE_ADDR'];
            $dataLogin['created_at'] = now();
            $dataLogin['updated_at'] = now();
            \DB::table('h_login_histories')->insert($dataLogin);
