...
trait InsertTrait
{
    ...
    public function __insert(Request $request, $table)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $form_contacts = [];
            foreach ($data as $key => $value) {
                if (strpos($key, 'form_contact') === 0) {
                    $form_contacts[$key] = $value;
                    unset($data[$key]);
                }
            }
            if (isset($data['slug'])) {
                ...
                try {
                    ...
                    if ($errorCode == 1062) {
                        \Session::flash('messageNotify', 'Lỗi trùng lặp dữ liệu. ' . $e->getPrevious()->getMessage());
                        return \Support::json(['code' => 100, 'message' => 'Lỗi trùng lặp dữ liệu. ' . $e->getPrevious()->getMessage()]);
                    }
                    \Session::flash('messageNotify', 'Đã có lỗi xảy ra');
                    return \Support::json(['code' => 100, 'message' => 'Đã có lỗi xảy ra. ' . $e->getPrevious()->getMessage()]);
                }
            }
            \DB::beginTransaction();
    ...
    private function __insertContact($itemId, $pivots, $table)
    {
        foreach ($pivots as $key => $pivot) {
            ...
            $pivotValues = json_decode($pivot, true);
            foreach ($pivotValues as $value) {
                if (count($value[$field_main]) == 0) {
                    continue;
                }
                $dataItem = [];
                foreach ($value as $keyValue => $item) {
                    $dataItem[$keyValue] = $keyValue == $field_main ? json_encode($item, JSON_UNESCAPED_UNICODE) :  $item;
                }
                \DB::table($table_form)->insert(array_merge($dataItem, [
                    $filed_target_id => $itemId,
                    $field_table => $table,
                    'created_at' => new \DateTime,
                    'updated_at' => new \DateTime,
                ]));
