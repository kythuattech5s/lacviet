...
class PageController extends BaseAdminController
{
    ...
    public function loadPage(Request $request, $id)
    {
        ...
        if (!file_exists(public_path(self::ROOT . $page->url))) {
            file_put_contents(public_path(self::ROOT . $folder), '');
        }
        return file_get_contents(public_path(self::ROOT . $folder));
    }
    ...
    public function savePage(Request $request, $id)
    {
        ...
        $currentItem = Page::find($id);
        $data = $request->all();
        $content = json_encode($data['data'], true);
        file_put_contents(public_path(self::ROOT . $folder), $content);
        $render = view('pg::render', compact('data', 'currentItem'))->render();
        file_put_contents(public_path(self::ROOT . $folder . '.html'), $render);
        return true;
    }

    public function getDataPage(Request $request, $id)
    {
        $page = Page::find($id);
