...
class Helper
{
    ...
    public static function addComment($request, $checkShop = false, $nameRating = 'rate')
    {
        ...
        foreach ($request->all() as $key => $value) {
            if (strpos($key, 'plus_') !== false) {
                $key = str_replace('plus_', '', $key);
                $comment->$key = $value;
            }
        }
        $comment->act = config('cmrsc_comment.active');
    ...
    private static function renderComment($comments)
    {
        ...
        $currentItem->id = request()->input('map_id');
        $html = view('commentRS::paginate.item', compact('comments', 'map_table', 'currentItem'))->render();
        return response([
            'code' => 200,
            'html' => $html,
            'lastPage' => $comments->lastPage(),
