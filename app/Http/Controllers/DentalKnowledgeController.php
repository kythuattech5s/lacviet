...
use App\Models\{DentalKnowledge};
use App\Models\Services;

class DentalKnowledgeController extends Controller
{	
    public function view($request, $route, $link){
        if (!\Auth::guard('h_users')->check()) {
            $currentItem = DentalKnowledge::slug($link)->with('ratings')->act()->publish()->first();
        }else {
            $currentItem = DentalKnowledge::slug($link)->with('ratings')->first();
        }
        ...
        $listNewsSelect = DentalKnowledge::whereIn('id',$listIdNewSelect)->act()->publish()->get()->all();
        $dataContent = \Support::createdTocContent($currentItem->content);
        $listMostViewNews = DentalKnowledge::act()->where('id','!=',$currentItem->id)->publish()->orderBy('count_view','desc')->limit(5)->get();
        $listHotNews = DentalKnowledge::act()->where('hot',1)->where('id','!=',$currentItem->id)->publish()->orderBy('time_published','desc')->limit(6)->get();
