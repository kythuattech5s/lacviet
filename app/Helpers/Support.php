...
class Support
{
    ...
    public static function showMenuRecursive($menus)
    {
        if ($menus->count() > 0) {
            ...
            foreach ($menus as $menu) {
                ...
                echo $aStartLink;
                if ($menu->icon != '') {
                    echo '<img src="' . \FCHelper::eimg2($menu, 'icon', '350x0') . '" alt="' . \Support::show($menu, 'name') . '" title="' . \Support::show($menu, 'name') . '"/>';
                }
    ...
    public static function createdTocContent($content)
    {
        ...
        foreach ($listImg as $itemImg) {
            $sourceShow = $itemImg->src;
            if (!\Str::contains($sourceShow, 'http')) {
                $sourceShow = url()->to($sourceShow);
            }
            $itemImg->src = $sourceShow;
            $itemImg->parent()->innertext = str_replace($itemImg->outertext, "<a href='" . $sourceShow . "'' data-fancybox='gallery' data-animation-effect='fade'>" . $itemImg->outertext . "</a>", $itemImg->parent()->innertext);
        }
        $html->save();
        $data['toc'] = $toc;
        $data['content'] = $html;
    ...
    public static function showContentHasForm($content, $map_table, $map_id)
    {
        ...
        $allKey = $allKey[0] ?? [];
        foreach ($allKey as $itemKey) {
            if (strpos($itemKey, 'el_form_contact') !== false) {
    ...
    {
        return isset($item->nofollow) && (int)$item->nofollow == 1 ? 'rel="nofollow"' : '';
    }
    ...
    public static function analysisItemImage($itemImage)
    {
        $name = $itemImage->name ?? \SettingHelper::getSetting('site_name', 'Bách Hợp Tiểu Thuyết');
        $ret = [];
        $ret['title'] = '';
        $ret['alt'] = '';
        $imageInfo = static::extractJson($itemImage->img);
